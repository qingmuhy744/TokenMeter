...
如果你不想克隆整个仓库，可以直接从 GitHub 提取 `docker-compose.yml` 文件：

```bash
# 创建目录并进入
mkdir tokenmeter && cd tokenmeter

# 仅从 GitHub 提取 docker-compose.yml 文件
curl -O https://raw.githubusercontent.com/qingmuhy744/TokenMeter/main/docker-compose.yml

# 或者使用 git (如果你配置了 SSH)
# git archive --remote=git@github.com:qingmuhy744/TokenMeter.git HEAD docker-compose.yml | tar -x
```
