...
const getHeatmapColor = (value: number | null, type: 'ttft' | 'tps' | 'degradation') => {
  ...
  // Using OKLCH colors for better accessibility and aesthetics
          ...
          <Table>
            ...
            <TableBody>
              {table.getRowModel().rows.map(row => (
                <TableRow key={row.id} className="group hover:bg-primary/5 transition-colors border-b border-border/50 last:border-0">
                  {row.getVisibleCells().map((cell, index) => (
                    <TableCell 
                      key={cell.id} 
                      className={cn(
                        "px-4 py-2 align-middle transition-colors",
                        index === 0 && "sticky left-0 bg-background/95 backdrop-blur-md z-10 border-r border-border/50 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.1)] group-hover:bg-[oklch(0.97_0_0)] dark:group-hover:bg-[oklch(0.25_0_0)]"
                      )}
                    >
